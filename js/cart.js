...
export function initCartPage() {
    ...
    if (!cartContainerSection || !emptyCartMessage)
        return;
    renderCart();
...
function renderCart() {
    ...
    if (!cartContainerSection || !emptyCartMessage || !itemsWrapper)
        return;
    const cart = getCart();
    if (cart.length === 0) {
...
function bindCartEvents() {
    ...
    wrapper.addEventListener('click', (e) => {
        ...
        if (target.closest('.btn-plus')) {
            const currentQty = parseInt(row.querySelector('.qty-val').value);
...
function updateTotalsUi() {
    ...
    if (shippingEl)
        shippingEl.textContent = totals.shipping > 0 ? '$' + totals.shipping : 'Free';
    if (totalEl)
